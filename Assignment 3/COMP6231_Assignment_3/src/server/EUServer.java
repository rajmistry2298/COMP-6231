...
public class EUServer implements GameServer{
	
	public String serverLocation = LocationConstants.EUROPE;
	...
	public EUServer() throws RemoteException,IOException{
	...
	public static void main(String[] args) {
		try {
			...
			final EUServer gameServerImpl = new EUServer();
            
            /*Binding it to service registry --> WSDL file is created*/
        	Endpoint endpoint = Endpoint.publish(LocationConstants.EUROPE_ADDRESS, gameServerImpl);
        	
        	System.out.println("Europe Service Is Published : " + endpoint.isPublished());
        	writeLog("Europe Service Is Published : " + endpoint.isPublished());
            
            /*Start UDP server for EU*/
            new Thread(() -> {
	...
	private static void startUDPServer(final EUServer gameServer) {
    ...
    private static void processUDPRequest(final EUServer gameServer, final DatagramPacket dp) {
    	byte[] response;
        DatagramSocket socket = null;
